var ImageColorMask = function(colors, opts){
	...
	var setBit = function(number, location, bit) {
		return (number & ~(1 << (8-location-1))) | (bit << (8-location-1));
	};
	var setBit2 = function(number, location, bit) {
		return (number & ~(1 << (opts.lengthSize-location-1))) | (bit << (opts.lengthSize-location-1));
	};
	...
	this.writeNumber2 = function(number, size){
		var bits = getBitsFromNumber(number, size);
		var pos = 0;
		var mix = 0;
		if(opts.debug)console.log(bits.join(''));
		while(pos < bits.length && offset < this.colors.length){
			this.colors[offset] = setBit2(this.colors[offset], mix++, bits[pos++]);			
			/*while(mix < opts.mixCount && pos < bits.length){
				this.colors[offset] = setBit2(this.colors[offset], mix++, bits[pos++]);
			}*/
			if(opts.debug){
				for(var c=mix;c < 8;c++){
					this.colors[offset] = setBit2(this.colors[offset], c, 0);
				}
			}
			offset ++;
			mix = 0;
	...
	this.readNumber2 = function(size){		
		var bits = opts.debug ? [] : null;
		var color = opts.debug ? [] : null;
		var pos = 0;
		var number = 0;
		var mix = 0;
		while(pos < size && offset < this.colors.length){
			var bit = getBit(this.colors[offset], mix++);
				number = setBit2(number, pos++, bit);
				if(opts.debug)bits.push(bit);
				if(opts.debug)color.push(this.colors[offset]);
			/*while(mix < opts.mixCount && pos < size){
				bit = getBit(this.colors[offset], mix++);
				number = setBit(number, pos++, bit);
				if(opts.debug)bits.push(bit);
		...
		if(opts.debug)console.log(bits.join(''));
		if(opts.debug)console.log(color.join(','));
		return number;
	}

	this.readNumber22 = function(size){		
		var bits = opts.debug ? [] : null;
		var pos = 0;
		var number = 0;
		var mix = 0;
...
ImageMask.prototype.revealFile = function(canvas){
	...
	*/
	...
	var fileLength = colorMask.readNumber2(this.opts.lengthSize);
	...
	for (var i = 0; i < fileLength; i++) {
		var b = colorMask.readNumber22(8);
		data[i] = b;
	}
